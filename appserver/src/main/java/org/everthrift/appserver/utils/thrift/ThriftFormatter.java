...
public class ThriftFormatter {
    ...
    public String formatClass(Class<? extends TBase> cls) throws ClassNotFoundException {
        ...
        for (TFieldIdEnum f : fields) {
            ...
            sb.append(String.format("\t<li style=\"list-style-type: none;\">%d:%s %s %s</li>\n", f.getThriftFieldId(),
                                    formatTypeName(fmd.valueMetaData), formatRequirementType(fmd.requirementType), fmd.fieldName));
        }
    ...
    private String linkClass(Class cls) {
        return String.format("<a href=\"%s\">%s</a>", basePath + "struct/" + URLEncoder.encode(cls.getCanonicalName()) + "/",
                             cls.getSimpleName());
    }

    private String linkEnum(Class cls) {
        return String.format("<a href=\"%s\">%s</a>", basePath + "enum/" + URLEncoder.encode(cls.getCanonicalName()) + "/",
                             cls.getSimpleName());
    }
