...
public class MappingManager {
    ...
    private final EntityMapper.Factory factory;

    private final EntityParser entityParser;

    public final boolean isCassandraV1;
     ...
     * {@link #MappingManager(Session, ProtocolVersion)} if that is a problem
     * for you).
     *
     ...
     * Creates a new {@code MappingManager} using the provided {@code Session}.
     * <p/>
     * This constructor is only provided for backward compatibility: before
     * 2.1.7, {@code MappingManager} could be built from an uninitialized
     * session; since 2.1.7, the mapper needs to know the active protocol
     * version to adapt its internal requests, so
     * {@link #MappingManager(Session)} will now initialize the session if
    ...
    public MappingManager(Session session, ProtocolVersion protocolVersion, EntityMapper.Factory factory, EntityParser entityParser) {
        ...
        // This is not strictly correct because we could connect to C* 2.0 with
        // the v1 protocol.
        // But mappers need to make a decision early so that generated queries
        // are compatible, and we don't know in advance
     ...
     * <p/>
     * Note that you can get obtain the {@code Cluster} object corresponding to
     * that session using {@code getSession().getCluster()}.
     * <p/>
    ...
    /**
     * Creates a {@code Mapper} for the provided class (that must be annotated
     * by a {@link Table} annotation).
     * <p/>
     * The {@code MappingManager} only ever keeps one Mapper for each class, and
     * so calling this method multiple times on the same class will always
     * return the same object.
     * <p/>
     * If the type of any field in the class is an {@link UDT}-annotated
     * classes, a codec for that class will automatically be created and
    ...
    /**
     * Creates a {@code TypeCodec} for the provided class (that must be
     * annotated by a {@link UDT} annotation).
     * <p/>
     * This method also registers the codec against the underlying
     * {@code Cluster}. In addition, the codecs for any nested UDTs will also be
     * created and registered.
     * <p/>
     * You don't need to call this method explicitly if you already call
    ...
    /**
     * Creates an accessor object based on the provided interface (that must be
     * annotated by a {@link Accessor} annotation).
     * <p/>
     * The {@code MappingManager} only ever keep one Accessor for each class,
    ...
    <T> TypeCodec<T> getUDTCodec(Class<T> mappedClass) {
        ...
        if (codec == null) {
            synchronized (udtCodecs) {
                ...
                if (codec == null) {
                    codec = entityParser.parseUDT(mappedClass, factory, this);
