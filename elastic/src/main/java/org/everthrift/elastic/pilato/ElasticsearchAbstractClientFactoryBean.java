    ...
    private void initMappings() throws Exception {
        ...
        if (mappings != null && mappings.length > 0) {
            ...
            for (String index : indexes.keySet()) {
                IndexElasticsearchUpdater.createIndex(client, classpathRoot, indexPrefix, index, forceMapping);
                if (mergeSettings) {
                    IndexElasticsearchUpdater.updateSettings(client, classpathRoot, indexPrefix, index);
                }
                ...
                for (Iterator<String> iterator = mappings.iterator(); iterator
                    ...
                    TypeElasticsearchUpdater.createMapping(client, classpathRoot, indexPrefix, index, type, mergeMapping);
                }
            }
        }
    }
    ...
    private void initAliases() throws Exception {
        if (aliases != null && aliases.length > 0) {
            for (int i = 0; i < aliases.length; i++) {
                ...
                createAlias(client, indexPrefix + alias, indexPrefix + index);
            }
        }
    }
