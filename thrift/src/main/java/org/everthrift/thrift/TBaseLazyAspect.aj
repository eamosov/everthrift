...
public aspect TBaseLazyAspect {
	...
	Object around(TBaseLazyModel acc, TBaseLazyModel that): this(acc) && execution(boolean TBaseHasLazyModel+.equals(TBase+)) && args(that){
		...
		if (log.isTraceEnabled()){
			final String s1 = acc.getThriftData() == null ? acc.toString() : String.format("%s(<compressed>, length:%d)", acc.getClass().getSimpleName(), acc.getThriftData().length); 
			final String s2 = that.getThriftData() == null ? that.toString() : String.format("%s(<compressed>, length:%d)", that.getClass().getSimpleName(), that.getThriftData().length);
